...
<body>
    ...
    <script>
        $(document).ready(function () {
            ...
            // Collapse menu if clicked outside of navbar when open
            $(document).on('click', function (event) {
                var clickTarget = $(event.target);
                var isNavbarOpen = $('.navbar-collapse').hasClass('show');
                if (isNavbarOpen && !clickTarget.closest('.navbar-collapse').length && !clickTarget.closest('.navbar-toggler').length) {
                    $('.navbar-collapse').collapse('hide');
                }
            });
        });
